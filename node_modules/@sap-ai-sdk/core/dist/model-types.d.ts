type LiteralUnion<T extends U, U = string> = T | (U & Record<never, never>);
/**
 * Azure OpenAI models for chat completion.
 */
export type AzureOpenAiChatModel = LiteralUnion<'gpt-5' | 'gpt-5-mini' | 'gpt-5-nano' | 'gpt-5.2' | 'gpt-5.4' | 'gpt-5.4-nano' | 'gpt-5.5' | 'gpt-5.1' | 'gpt-5.6-sol' | 'gpt-5.6-terra' | 'gpt-5.6-luna'>;
/**
 * Azure OpenAI models that exclusively support the Responses API.
 * Models that support both chat completions and the Responses API (e.g. gpt-5.4) are listed under {@link AzureOpenAiChatModel}.
 */
export type AzureOpenAiResponsesModel = LiteralUnion<'gpt-5.3-codex'>;
/**
 * Azure OpenAI models for embedding.
 */
export type AzureOpenAiEmbeddingModel = LiteralUnion<'text-embedding-3-small' | 'text-embedding-3-large'>;
/**
 * Azure OpenAI models for the Realtime (speech-to-speech) API.
 * Currently only `gpt-realtime` is supported by SAP AI Core; other realtime scenarios (e.g. transcription, translation) are not fully supported.
 */
export type AzureOpenAiRealtimeModel = LiteralUnion<'gpt-realtime'>;
/**
 * GCP Vertex AI models for chat completion.
 */
export type GcpVertexAiChatModel = LiteralUnion<'gemini-2.5-flash-lite' | 'gemini-3.1-flash-lite' | 'gemini-3.5-flash' | 'gemini-3.5-flash-lite'>;
/**
 * GCP Vertex AI models for embedding.
 */
export type GcpVertexAiEmbeddingModel = LiteralUnion<'gemini-embedding'>;
/**
 * AWS Bedrock models for chat completion.
 */
export type AwsBedrockChatModel = LiteralUnion<'amazon--nova-pro' | 'amazon--nova-lite' | 'amazon--nova-micro' | 'anthropic--claude-4.5-haiku' | 'anthropic--claude-4.6-opus' | 'anthropic--claude-4.5-sonnet' | 'anthropic--claude-4.6-sonnet' | 'anthropic--claude-4.7-opus' | 'anthropic--claude-4.5-opus' | 'anthropic--claude-4.8-opus'>;
/**
 * AWS Bedrock models for embedding.
 */
export type AwsBedrockEmbeddingModel = LiteralUnion<'amazon--titan-embed-text'>;
/**
 * Perplexity models for chat completion.
 */
export type PerplexityChatModel = LiteralUnion<'sonar' | 'sonar-pro'>;
/**
 * AI Core open source models for chat completion.
 */
export type AiCoreOpenSourceChatModel = LiteralUnion<'cohere--command-a-reasoning' | 'mistralai--mistral-medium-instruct' | 'mistralai--mistral-small' | 'mistralai--mistral-medium' | 'sap-abap-1'>;
/**
 * AI Core sap managed model for embedding.
 */
export type AiCoreOpenSourceEmbeddingModel = LiteralUnion<'nvidia--llama-3.2-nv-embedqa-1b'>;
/**
 * SAP RPT models.
 * @remarks `sap-rpt-1-small` and `sap-rpt-1-large` are deprecated and will be removed 2026-12-31. Use `sap-rpt-1.5` or `sap-rpt-1.5-large` instead.
 */
export type SapRptModel = LiteralUnion<'sap-rpt-1.6' | 'sap-rpt-1.6-large' | 'sap-rpt-1.5' | 'sap-rpt-1.5-large' | 'sap-rpt-1-small' | 'sap-rpt-1-large'>;
/**
 * Models supported for LLM batch processing.
 * The full list is managed by running the `update-models` skill.
 */
export type LlmBatchModel = LiteralUnion<'gpt-4.1' | 'gpt-4.1-mini' | 'gpt-4.1-nano' | 'gpt-5' | 'gpt-5.4' | 'gpt-5.4-mini'>;
export {};
//# sourceMappingURL=model-types.d.ts.map