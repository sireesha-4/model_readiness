...
function userId(jwtPayload) {
    ...
    logger.debug(`JWT user identifier is: ${id} (from ${jwtPayload.user_id ? 'user_id (XSUAA)' : 'user_uuid (IAS)'}).`);
    return id;
}
/**
 * @internal
...
function getTenantId(jwt) {
    ...
    logger.debug(`JWT zid is: ${decodedJwt.zid}, app_tid is: ${decodedJwt.app_tid}, zone_uuid is: ${decodedJwt.zone_uuid}.`);
    return (decodedJwt.zid || decodedJwt.app_tid || decodedJwt.zone_uuid || undefined);
}
/**
 ...
 * @internal
 */
function isIasToken(decodedJwt) {
    if (!decodedJwt.iss) {
        return false;
    }
    try {
        ...
        return (hostname.endsWith('.accounts.ondemand.com') ||
...
function getSubdomain(jwt) {
    ...
    return (decodedJwt?.ext_attr?.zdn ||
        (isIasToken(decodedJwt) ? undefined : (0, subdomain_replacer_1.getIssuerSubdomain)(decodedJwt)));
}
/**
 * @internal
